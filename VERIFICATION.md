...
This log records the manual source check for the 96 records in [`data/venues.json`](data/venues.json) — 20 launch records, 50 added in the second verification pass, and 26 merged from a parallel verification pass (PR #2). The snapshot date is **September 5, 2026**. Each row below is linked to the venue-controlled page used for the name, address, and hours. No third-party directory is used as the inclusion source.
...
## Parallel verification pass (records 71–96), merged from PR #2

A second Arena session ran an independent expansion against the same 20-record base and shipped 34 new records in PR #2. This merge keeps 26 of them verbatim below. The other 8 (SF Eagle, Trick Dog, Local Edition, Presidio Bowl, Rickhouse, Madrone Art Bar, Bender's Bar & Grill, Gino & Carlo) duplicate records already documented in the second-pass section above, so those checked versions are the ones carried in the data file. Entry text below is unedited from the parallel pass; its original section was titled "Record-by-record log" and it also documented a shortfall against its own 50-record target.

### 71. Mel's Drive-In — 4th & Mission
...
### 72. Mel's Drive-In — Lombard Street
...
### 73. Mel's Drive-In — Geary Boulevard
...
### 75. Starlite at the Beacon Grand
...
### 79. El Farolito — 4817 Mission Street
